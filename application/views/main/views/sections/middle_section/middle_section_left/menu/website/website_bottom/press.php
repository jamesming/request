...
<div  class='sub_menu_area_div' >
	<ul  class='sub_menu_area' >
		...
		<li submenu_item="description" ><a href='<?php echo base_url()    ?>index.php/main/index/menu/website/press/description' submenu_item='description'>Description</a>&nbsp;</li>
		<li submenu_item="picture"  class='last_submenu_item ' >
			<div  class='center_this '>
					<div  class=' float_left' >
						<input  class='tab_include_checkboxes ' field="press_picture_checkbox" type="checkbox" value=""   style='border:0'  >
					</div>
					<div  class='float_left'   style='margin-left:4px'  >
						<a href='<?php echo base_url()    ?>index.php/main/index/menu/website/press/picture' submenu_item='picture'>Picture</a>
					</div>
			</div>
		</li>
...
<script type="text/javascript" language="Javascript">
	...
	$(document).ready(function() {
		...
		$('.sub_menu_area_div ul.sub_menu_area li:not(li.line)').unbind('click').css({cursor:'default'})
			...
			<?php
			if( $website_data['users'][0]->press_picture_checkbox == 1){
			?>
				$('input[field=press_picture_checkbox]').attr('checked','checked');			
			<?php     
			}else{
			?>
				$('input[field=press_picture_checkbox]').removeAttr('checked');			
			<?php     	
			};
			?>
	})
